...
# Public GitHub Repo: 
# https://github.com/jeffreyCarlLong/hsct-shiny
...
# write.table(cytomegalovirus, "cytomegalovirus.txt", sep = "\t")
# cytomegalovirus <- read.delim("cytomegalovirus.txt", sep = "\t")
...
# Read in custom data dictionary for trial
dict <- read.delim("hsct_data_dictionary.tsv", sep = "\t")


# Define UI for application that draws a histogram
ui <- fluidPage(
  titlePanel(div(h2("Retrospective Cohort Study of Myeloablative Allogeneic Hematopoietic Stem Cell Transplant."),
                 h4("For comments/issues with app, please email Jeff: jeffrey.c.long@gsk.com"))),
  tabsetPanel(
    tabPanel("Study Details", fluid = TRUE,
             mainPanel(
               h3("Study Abstract"),
               h4("This data set contains 64 consecutive patients who underwent T-cell replete, matched sibling donor reduced-intensity conditioning allogeneic hematopoietic stem cell transplant. A number of demographic, baseline and transplant characteristics were also collected. The dataset is cleaned and relatively complete. There are no outliers or data problems."),
               h3("Indications: "),
               verbatimTextOutput("indications"),
               h3("Primary Risk Factor: "),
               h4("The number of activating killer immunoglobulin-like receptors (aKIRs: 1-4 vs. 5-6)"),
               h3("Primary Outcome: "),
               h4("The primary outcome is presence of and time to cytomegalovirus reactivation. "),
               h3("Background: "),
               ...
               h3("References: "),
               h4("These are data from a study by Sobecks et al, Cytomegalovirus Reactivation After Matched Sibling Donor Reduced-Intensity Conditioning Allogeneic Hematopoietic Stem Cell Transplant Correlates With Donor Killer Immunoglobulin-like Receptor Genotype, Exp Clin Transplant 2011; 1: 7-13.")
               )
    ),
          ...
          sidebarLayout(
             sidebarPanel(
               ...
               selectInput('sur_var', 'Factor of Survival', names(cytomegalovirus)[names(cytomegalovirus) != "time.to.transplant" & 
                                                                                     ...
                                                                                     names(cytomegalovirus) != "donor.cmv" & 
